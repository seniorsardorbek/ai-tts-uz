...
// Multilingual mode: no language_code, no transliteration.
// ElevenLabs v3 model auto-detects language from the text itself.
...
async function* streamOnce({ text, voice, mood }, { signal } = {}) {
  ...
  const resp = await fetch(url, {
    ...
    headers: {
      ...
      Accept: 'audio/mpeg',
      // Some datacenter IPs get Cloudflare's bot challenge ("Just a moment…")
      // when the request looks non-browser. A realistic User-Agent + Accept-*
      // headers reduce that. (If the IP is hard-flagged, route via a clean IP.)
      'User-Agent':
